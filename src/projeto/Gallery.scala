...
case class Gallery(gallery:Album){
  def insert(bm:String) : Album = Gallery.insert(bm,this.gallery)
  def searchImage(id:Int):Option[Image] = Gallery.searchImage(id, this.gallery)
  def swapImage(id1:Int, id2:Int ): Album = Gallery.swapImage(id1,id2,this.gallery)
  def delete (id:Int):Album = Gallery.delete(id,this.gallery)
  def next( id:Int): Image= Gallery.next(id,this.gallery)
  def previous( id:Int): Image= Gallery.previous(id,this.gallery)
}
...
//Verificar quando  o album é vazio
  ...
  def next( id:Int, album: Album): Image={
    val index = album.indexWhere(x => { x == id } )
...
//editar informação se quisermos
